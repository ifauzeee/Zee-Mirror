...
func TestFindDownloadedFile(t *testing.T) {
	t.Run("SingleFile", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), []byte("data"), 0644))
	...
	t.Run("MultipleFiles_ReturnsLargest", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "small.mp4"), []byte("small"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "large.mp4"), make([]byte, 1000), 0644))
	...
	t.Run("SkipsAria2Files", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4.aria2"), []byte("data"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), []byte("data"), 0644))
	...
	t.Run("SkipsPartFiles", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4.part"), []byte("data"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), []byte("data"), 0644))
	...
	t.Run("SkipsTempFiles", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "download.temp"), []byte("data"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), []byte("data"), 0644))
	...
	t.Run("SkipsTorrentFiles", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "file.torrent"), []byte("data"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), []byte("data"), 0644))
	...
	t.Run("QualityAudio_SelectsAudioFile", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), make([]byte, 500), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "audio.mp3"), make([]byte, 100), 0644))
	...
	t.Run("QualityVideo_SelectsVideoFile", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "audio.mp3"), make([]byte, 500), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mp4"), make([]byte, 100), 0644))
	...
	t.Run("MultipleAudioFormats", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "song.flac"), make([]byte, 200), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "song.opus"), make([]byte, 100), 0644))
	...
	t.Run("MultipleVideoFormats", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.mkv"), make([]byte, 200), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "video.webm"), make([]byte, 100), 0644))
...
func TestGetLargest(t *testing.T) {
	...
	t.Run("MultiplePaths", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(small, []byte("hi"), 0644))
		require.NoError(t, os.WriteFile(large, make([]byte, 100), 0644))
