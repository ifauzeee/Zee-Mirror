        ...
        >
            <div className="flex flex-col md:flex-row items-center justify-between glass-card p-10 rounded-[3rem] shadow-2xl gap-8 relative overflow-hidden">
                {isDragging && (
                    <div className="absolute inset-0 bg-primary/10 backdrop-blur-sm flex items-center justify-center z-50 border-4 border-dashed border-primary/50 rounded-[3rem] animate-pulse">
                        <div className="flex flex-col items-center space-y-4">
                            ...
                            <p className="font-black text-primary uppercase tracking-widest text-xl">Drop to Deploy Resource</p>
                        </div>
                    </div>
                )}
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button onClick={() => fetchExplorer(explorerPath)} className="p-4 bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-primary rounded-2xl transition-all shadow-sm border border-slate-100 dark:border-white/5">
                        {isUploading ? <Loader2 size={24} className="animate-spin" /> : <RefreshCcw size={24} />}
                    </button>
                </div>
            </div>
            ...
            <div className="glass-card rounded-[3.5rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-white/5 bg-white/80 dark:bg-zinc-900/40 backdrop-blur-3xl px-2">
                ...
                <div className="divide-y divide-slate-100 dark:divide-white/5">
                    {explorerFiles.length === 0 && (
                        <div className="px-10 py-20 text-center text-slate-400 italic font-bold uppercase tracking-widest opacity-50">Empty Sector - Ready for Data Propagation</div>
                    )}
                            ...
                            <div className="col-span-6 md:col-span-7 flex items-center space-x-6 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center space-x-3 mt-1.5">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate opacity-70 font-mono italic whitespace-nowrap overflow-hidden">NODE-ID: {file.name.slice(0, 8).toUpperCase()}</span>
                                    </div>
                                </div>
                            </div>
