...
import { Settings as SettingsIcon, Save, Folder, TrendingUp, ShieldCheck, Terminal, Cpu, Database, Bell, RefreshCw, Zap } from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto pb-24 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-6 bg-white/40 dark:bg-white/5 p-10 rounded-[3rem] border border-white/20 dark:border-white/5 backdrop-blur-md">
                <div className="flex items-center space-x-6">
                    ...
                    <div>
                        <h2 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white">Core Control</h2>
                        <p className="text-[10px] uppercase font-black tracking-[0.3em] text-primary/60 mt-1">Zee-Mirror System Administration</p>
                    </div>
                </div>
                <div className="flex items-center space-x-4">
                    <div className="px-6 py-3 bg-white/80 dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm flex items-center space-x-3">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Node Status: Active</span>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-7 space-y-10">
                    ...
                    <div className="glass-card p-10 rounded-[3.5rem] relative overflow-hidden group">
                        <div className="flex items-center space-x-4 mb-10">
                            ...
                            <h3 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white uppercase">Bot Behavior</h3>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center justify-between p-7 rounded-[2.5rem] bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-primary/30 transition-all duration-500">
                                <div className="flex-1 pr-8">
                                    <h4 className="font-bold text-slate-900 dark:text-white">Auto-Cleanup Protocol</h4>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">Automatically purge status messages from Telegram once a task completes.</p>
                                </div>
                                <button
                                ...
                                >
                                    <div className={`w-6 h-6 bg-white rounded-full transition-all duration-300 shadow-lg ${settings.AutoDeleteMessages ? 'translate-x-8' : 'translate-x-0'}`} />
                                </button>
                            </div>

                            <div className="p-7 rounded-[2.5rem] bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                                <div className="flex items-center space-x-3 mb-6">
                                    <h4 className="font-bold text-slate-900 dark:text-white">Default Transmission Mode</h4>
                                </div>
                        ...
                        <button onClick={handleUpdateSettings} className="w-full mt-10 bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-6 rounded-[2.5rem] font-black text-sm uppercase tracking-widest flex items-center justify-center space-x-3 hover:scale-[0.98] active:scale-[0.95] transition-all shadow-xl">
                            <Save size={20} />
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        <div className="glass-card p-10 rounded-[3rem] border-emerald-500/10 dark:border-emerald-500/5">
                            <div className="p-3 w-fit bg-emerald-500/10 text-emerald-500 rounded-2xl mb-6"><Cpu size={24} /></div>
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">Architecture</p>
                            <p className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-widest">Go 1.25.7 Pure</p>
                        </div>
                        <div className="glass-card p-10 rounded-[3rem] border-blue-500/10 dark:border-blue-500/5">
                            <div className="p-3 w-fit bg-blue-500/10 text-blue-500 rounded-2xl mb-6"><Database size={24} /></div>
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">Data Matrix</p>
                            <p className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-widest">SQLite CGO-Free</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-5">
                    <div className="glass-card p-12 rounded-[4rem] h-full flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-10 relative z-10">
                            <div className="flex items-center space-x-4">
                                <div className="p-3 bg-indigo-500/10 text-indigo-500 rounded-2xl"><Terminal size={24} /></div>
                                <h3 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white uppercase">Manifest</h3>
                            </div>
                            <button onClick={fetchRawConfig} className="p-3 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all text-slate-500">
                            ...
                            <div className="flex-1 flex flex-col space-y-8">
                                <div className="relative group flex-1 min-h-[450px]">
                                    <textarea
                                        ...
                                        spellCheck="false"
                                    />
                                    <div className="absolute bottom-6 right-6 px-3 py-1 bg-white/5 backdrop-blur-md border border-white/10 rounded-full text-[9px] text-slate-500 font-black tracking-widest uppercase">READ_WRITE_OK</div>
                                </div>

                                <button onClick={handleUpdateRawConfig} className="w-full bg-indigo-600 text-white py-6 rounded-[2.5rem] font-black text-sm uppercase tracking-widest flex items-center justify-center space-x-3 hover:bg-indigo-500 hover:scale-[0.98] transition-all shadow-2xl shadow-indigo-500/20">
                                    <Save size={20} />
                                ...
                                <p className="text-center text-[9px] font-black text-slate-400 opacity-40 uppercase tracking-[0.2em] leading-relaxed px-6">
                                    System notice: Some environment variables may require a node restart to initialize the new cluster parameters.
                                </p>
                            </div>
                        )}
                    </div>
