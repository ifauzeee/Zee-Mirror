...
import { Clock, Trash2, Download, TrendingUp, Upload, CheckCircle, AlertCircle, RefreshCw } from 'lucide-react'
...
const TaskRow: React.FC<TaskRowProps> = ({ task, formatBytes, onCancel }) => {
  const getStatusIcon = () => {
    switch (task.status) {
      case 'downloading':
        return <Download size={18} className="text-blue-500 animate-pulse" />
      case 'uploading':
        return <Upload size={18} className="text-indigo-500 animate-pulse" />
      case 'queued':
      ...
      default:
        return <RefreshCw size={18} className="text-slate-400" />
    }
  }
        ...
        <div className="flex items-center space-x-6 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h4
              ...
              title={task.file_name || task.id}
            >
              {task.file_name || `Thread: ${task.id}`}
            </h4>
          </div>
        </div>
        {(task.status === 'downloading' || task.status === 'uploading' || task.status === 'queued') && (
          <button
